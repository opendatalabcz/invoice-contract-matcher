...
class TestContractSanitizer(TestCase):
    ...
    def test_sanitize_name_spaces_before(self):
        self.assertEqual("Ministerstvo kultury", ContractSanitizer.sanitize_name("      Ministerstvo kultury"))
        self.assertEqual("Ministerstvo vnitra", ContractSanitizer.sanitize_name("                  Ministerstvo vnitra"))
        self.assertEqual("Ministerstvo kultury", ContractSanitizer.sanitize_name(" Ministerstvo kultury"))
    ...
    def test_sanitize_name_spaces_between(self):
        ...
        self.assertEqual(result, ContractSanitizer.sanitize_name(test_case))
        self.assertEqual("Ministerstvo školství, mládeže",
                         ContractSanitizer.sanitize_name("Ministerstvo  školství,  mládeže"))
    ...
    def test_sanitize_ministry_name_empty_name(self):
        test_case = ContractSanitizer.sanitize_ministry_name(Contract(ministry_ico="00023671", ministry_name=None))
        res = Contract(ministry_ico='00023671', ministry_name='Ministerstvo kultury')
        self.assertEqual(res.ministry_name, test_case.ministry_name)
        self.assertEqual(res.ministry_ico, test_case.ministry_ico)

    def test_sanitize_ministry_name_part(self):
        test_case = ContractSanitizer.sanitize_ministry_name(Contract(ministry_ico="00023671", ministry_name='Ministerstvo'))
        res = Contract(ministry_ico='00023671', ministry_name='Ministerstvo kultury')
        self.assertEqual(res.ministry_name, test_case.ministry_name)
        self.assertEqual(res.ministry_ico, test_case.ministry_ico)

    def test_sanitize_ministry_name_with_additional_text(self):
        test_case = ContractSanitizer.sanitize_ministry_name(Contract(ministry_ico="00023671", ministry_name='Česká republika - Ministerstvo kultury'))
        res = Contract(ministry_ico='00023671', ministry_name='Ministerstvo kultury')
        self.assertEqual(res.ministry_name, test_case.ministry_name)
        self.assertEqual(res.ministry_ico, test_case.ministry_ico)

    def test_sanitize_ministry_name_not_ministry(self):
        test_case = ContractSanitizer.sanitize_ministry_name(Contract(ministry_ico="00023600", ministry_name='Test s.r.o.'))
        res = Contract(ministry_ico='00023600', ministry_name='Test s.r.o.')
        self.assertEqual(res.ministry_name, test_case.ministry_name)
        self.assertEqual(res.ministry_ico, test_case.ministry_ico)

    def test_sanitize_ministry_name_ico_none(self):
        test_case = ContractSanitizer.sanitize_ministry_name(Contract(ministry_ico=None, ministry_name='Test s.r.o.'))
    ...
    def test_find_ico_for_name(self):
        # jedno ico
        ico = ContractSanitizer.find_ico_for_name("AURIGA SYSTEMS S.R.O.")
        self.assertEqual("28871235", ico)

        # vice vysledku
        ico = ContractSanitizer.find_ico_for_name("tes")
        self.assertIsNone(ico)

        # fyzicka osoba
        ico = ContractSanitizer.find_ico_for_name("MÜLLNEROVÁ ZDISLAVA")
        self.assertIsNone(ico)